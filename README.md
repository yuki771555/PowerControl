...
You can also run `PowerControl.bat`. It requests administrator privileges, rebuilds `PowerControl.exe` first so stale local builds are replaced, then starts the native app.
...
PowerControl.bat     Admin launcher batch file (rebuilds and starts the native app; falls back to the legacy .ps1)
